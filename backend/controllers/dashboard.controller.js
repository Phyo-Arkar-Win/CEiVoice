...
export const getAssigneeDashboardData = async (req, res) => {
    try {
        ...
        const tickets = await Ticket.find({ assignees: { $in: [assigneeId] }, status: { $in: activeStatus } });
...
export const getAdminDashboardData = async (req, res) => {
    try {
        ...
        const ticketsCreated = await Ticket.countDocuments({ createdAt: { $gte: startDate } });

        // total active tickets
        const activeTickets = await Ticket.countDocuments({ status: { $in: ['New', 'Solving'] }, createdAt: { $gte: startDate } });
    
        // average resolution time for tickets in the period
        const solvedTickets = await Ticket.find({ status: 'Solved', updatedAt: { $gte: startDate}});

        let totalResolutionTime = 0;
        solvedTickets.forEach((solvedTicket) => {
            const resolutionTime = new Date(solvedTicket.updatedAt) - new Date(solvedTicket.createdAt);
            totalResolutionTime += resolutionTime;
        });
        const avgResolutionTimeMs = solvedTickets.length > 0 ? totalResolutionTime / solvedTickets.length : 0;
        ...
        const topCategory = await Ticket.aggregate([
            {
                $match: { // filter ticketes created in the period
                    createdAt: { $gte: startDate }
                } 
            },
            {
                $group: { // group by category and count
                    _id: "$category",
                    count: { $sum: 1 }
                }
            },
